...
const CustomCursor: React.FC = () => {
  ...
  return (
    <>
      {/* Dot */}
      <div 
        className="fixed top-0 left-0 w-2 h-2 bg-houzz-gold rounded-full z-[9999] pointer-events-none transition-transform duration-100 ease-out"
        style={{ transform: `translate3d(${position.x - 4}px, ${position.y - 4}px, 0)` }}
      />
      {/* Ring */}
      <div 
        className={`fixed top-0 left-0 border border-houzz-gold/50 rounded-full z-[9998] pointer-events-none transition-all duration-300 ease-out flex items-center justify-center ${
          isPointer ? 'w-16 h-16' : 'w-10 h-10'
        } ${isView ? 'bg-houzz-gold text-white border-none' : ''}`}
