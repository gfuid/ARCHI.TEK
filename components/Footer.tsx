    ...
    <footer className="bg-[#F8F7F2] text-[#1a1a1a] pt-24 pb-12 overflow-hidden relative border-t border-black/5">
      ...
      <div className="max-w-7xl mx-auto px-6 relative z-10">
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-16 mb-20">
            ...
            <Magnetic strength={0.1}>
              <Link to="/">
                <img
                  ...
                  className="h-24 md:h-32 w-auto mb-8 transition-opacity hover:opacity-80"
                />
              </Link>
            </Magnetic>
          ...
          <div className="lg:col-span-7 grid grid-cols-1 sm:grid-cols-3 gap-12">
            ...
            <div>
              ...
              <ul className="space-y-4 text-sm text-black/70 font-medium">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-x-8 gap-y-4">
                {EXPERTISE.map((item) => (
                  <Link
                    key={item.name}
                    to={item.link}
                    className="text-xs text-black/50 hover:text-[#C5A059] transition-colors flex items-center group"
                  >
        ...
        <div className="flex flex-col md:flex-row justify-between items-center gap-6">
          <div className="flex items-center space-x-6 text-[10px] font-bold tracking-widest text-black/30 uppercase">
          ...
          <div className="flex space-x-8">
            {['Instagram', 'LinkedIn', 'WhatsApp'].map((social) => (
              <Magnetic key={social} strength={0.2}>
                <a href="#" className="text-[10px] font-bold uppercase tracking-widest text-black/40 hover:text-[#C5A059] transition-colors">
                  {social}
                </a>
              </Magnetic>
            ))}
